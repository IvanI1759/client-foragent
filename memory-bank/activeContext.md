...
- Добавлен 5-й агент `consultant.js` (лид-квалификатор): отвечает только о проекте, отказ off-topic фразой «Я могу рассказать только о нашем проекте», CTA с ID и @katburova добавляется системой автоматически (а не Gemini).
...
- `message.js`: роутинг гостей на `askConsultant`, детектор «не могу помочь / не знаю» в ответах авторизованных агентов → добавляет «По этому вопросу напишите владельцу: @katburova».
...
2. [ ] Протестировать `consultant.js` локально: гость → ответ по проекту + CTA с ID и @katburova; off-topic → фиксированный отказ «Я могу рассказать только о нашем проекте»
...
- CTA с ID + @katburova добавляется кодом (после текста Gemini), не самим Gemini — гарантирует формат.
